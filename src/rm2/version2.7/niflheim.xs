...
void main(void) {
	...
	//       central area needs to be ice
	//       mist and fog effects in lower areas
	//       no settlements in lower areas
	//       create some non-player elevated areas with settlements to claim
	//       add some healing springs to be captured in the lower areas (where settlements were?)
	...
	rmAddConnectionTerrainReplacement(rampID, "cliffNorseA", "SnowGrass50");
	...
	for(i=1; <cNumberPlayers*12*mapSizeMultiplier) {
		...
		rmSetAreaTerrainType(patch, "SnowGrass75");
		rmAddAreaTerrainLayer(patch, "SnowB", 1, 2);
		rmAddAreaTerrainLayer(patch, "SnowGrass25", 0, 1);
	...
	for(i=1; <cNumberPlayers*12*mapSizeMultiplier){
		...
		rmAddAreaTerrainLayer(patch2, "SnowGrass25", 0, 3);
	...
	for(i=1; <cNumberPlayers){
		...
		rmSetAreaTerrainType(id2, "NorseRoadA");
		rmAddAreaTerrainLayer(id2, "SnowGrass50", 0, 1);
	...
	for(i=0; <numTries*mapSizeMultiplier){
		...
		rmSetAreaTerrainType(elevID, "SnowGrass50");
		rmAddAreaTerrainLayer(elevID, "SnowGrass25", 0, 1);
	...
	if(rmPlaceFairLocs()) {
		...
		rmAddAreaTerrainLayer(settleArea, "SnowGrass50", 2, 5);
		rmAddAreaTerrainLayer(settleArea, "SnowGrass25", 0, 2);
	...
	if(rmPlaceFairLocs()) {
		for(p = 1; <= cNumberNonGaiaPlayers){
			...
			rmAddAreaTerrainLayer(settlementArea, "SnowGrass50", 2, 5);
	...
	rmAddObjectDefItem(closeGoatsID, "boar", 2, 2.0);
	...
	rmAddObjectDefItem(stragglerTreeID, "pine tree", 1, 0.0);
	...
	if(cMapSize == 2) {
		for(i = 1; < cNumberPlayers) {
			...
			rmSetAreaForestType(playerForest2ID, "pine forest");
	...
	rmAddObjectDefItem(mediumGoatsID, "boar", 2, 1.0);
	...
	rmAddObjectDefItem(farHuntableID, "elk", rmRandInt(6,8), 4.0);
	...
	if(rmRandFloat(0,1) < 0.5){
		...
		rmAddObjectDefItem(bonusHuntableID, "boar", rmRandInt(4,5), 4.0);
	...
	rmAddObjectDefItem(farGoatsID, "boar", 1, 0.0);
	...
	rmAddObjectDefItem(randomTreeID, "pine tree", 1, 0.0);
	...
	if(cMapSize == 2){
		...
		rmAddObjectDefItem(giantHuntableID, "elk", rmRandInt(5,9), 5.0);
		...
		rmAddObjectDefItem(giantHerdableID, "boar", rmRandInt(2,3), 5.0);
