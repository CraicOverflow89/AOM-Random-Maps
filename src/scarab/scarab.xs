...
void main(void) {
	...
	int baseMountainWidth = 20;
	int connectionWidth = 25;
	if(cNumberTeams < 3) {
	...
	rmAddConnectionTerrainReplacement(connectionID, "cliffEgyptianA", "SandB");
	rmAddConnectionTerrainReplacement(connectionID, "cliffEgyptianB", "SandB");
	rmAddConnectionTerrainReplacement(connectionID, "cliffGreekA", "SandB");
	...
	if(secondConnectionExists == 1) {
		...
		rmAddConnectionTerrainReplacement(alternateConnection, "cliffGreekA", "SandA");
	...
	for(x = 1; < cNumberPlayers) {
		for(y = 0; < 3 * mapSizeMultiplier) {
			int id3 = rmCreateArea("snow patch" + x + y, rmAreaID("player" + x));
	...
	for(x = 1; < cNumberPlayers) {
		for(y = 0; < 3 * mapSizeMultiplier){
			int id2 = rmCreateArea("grass patch" + x + y, rmAreaID("player" + x));
	...
	// NOTE: could second arg not just be set by rmRandFloat(0,1) > 0.5 ?
	//       int startingGoldFairLocID = rmAddFairLoc("Starting Gold", rmRandFloat(0,1) > 0.5, false, 20, 21, 0, 15);
	if(rmPlaceFairLocs()) {
	...
	rmAddObjectDefItem(mediumDeerID, "gazelle", rmRandInt(6, 8), 3.0);
	...
	// NOTE: would be great if new library had a method to handle this repeated pattern
	//       forPlayers(logic: (index: int) -> Void) -> Void
	...
	rmAddObjectDefItem(farCowsID, "goat", 2, 4.0);
	...
	if(cMapSize == 2) {
		...
		for(x = 1; < cNumberPlayers) {
			rmPlaceObjectDefInArea(giantRelixID, x, rmAreaID("team"+rmGetPlayerTeam(x)), 1);
		}
	}
	...
	rmAddObjectDefItem(rockID4, "rock sandstone sprite", 1, 2.0);
	rmAddObjectDefItem(rockID4, "rock sandstone sprite", 2, 5.0);
	rmAddObjectDefItem(rockID4, "rock sandstone sprite", 3, 5.0);
